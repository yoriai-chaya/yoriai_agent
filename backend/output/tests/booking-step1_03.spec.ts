...
test.describe("Booking step-1 Basic Test", () => {
  test("Check check-in date operation", async ({ page }) => {
    ...
    const checkInButton = checkInCalendar.locator("table button", {
  ...
  test("Check check-out date operation", async ({ page }) => {
    ...
    const checkOutButton = checkOutCalendar.locator("table button", {
  ...
  test("Check select number of guests", async ({ page }) => {
    ...
    const guestOption = page.getByTestId("guest-option-3");
  ...
  test("Save screen capture", async ({ page }) => {
    ...
    await captureEvidenceScreenshot(page, "booking-step1.png");

    await compareWithSnapshot(page, "booking-step1.png");
  });
});
